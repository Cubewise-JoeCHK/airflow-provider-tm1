...
def get_fs(conn_id: str | None = None, storage_options: dict | None = None) -> AbstractFileSystem:
    ...
    # owns_service=True: the FS created the service, so it owns its lifecycle and
    # registers a finalizer to log out when the (GC'd) FS is no longer referenced.
    return TM1BlobStorage(tm1_service=tm1_service, owns_service=True, **(storage_options or {}))
...
class TM1BlobStorage(AbstractFileSystem):
    ...
    Service lifecycle: when ``owns_service=True`` (the default path via ``get_fs``),
    a :func:`weakref.finalize` callback logs out the TM1 service once the filesystem
    is garbage-collected. When the service was injected externally
    (``owns_service=False``), cleanup is left to the caller — so caching the FS does
    not prematurely close a shared connection.
    """

    protocol = ("tm1",)

    def __init__(self, tm1_service: TM1Service, owns_service: bool = False, **kwargs):
    ...
    @staticmethod
    def _cleanup_tm1_service(tm1_service: TM1Service) -> None:
        """Logout the TM1 service if it was created (owned) by this filesystem."""
        try:
            if tm1_service and hasattr(tm1_service, "logout"):
                log.debug("Finalizer cleaning up TM1 service")
                tm1_service.logout()
        except Exception as e:
            log.warning("Finalizer cleanup failed: %s", e)
