...
from mdxpy.mdx import ElementsHierarchySet, MdxAxis, MdxBuilder, Member
...
def build_mdx_statement(
    ...
    for row_subset in row_set.values():
        mdx_builder.add_hierarchy_set_to_row_axis(
            ElementsHierarchySet(
                *[Member(row_subset.dimension_name, row_subset.hierarchy_name, e) for e in row_subset.elements]
            )
        )
    ...
    for col_subset in col_set.values():
        mdx_builder.add_hierarchy_set_to_column_axis(
            ElementsHierarchySet(
                *[Member(col_subset.dimension_name, col_subset.hierarchy_name, e) for e in col_subset.elements]
            )
        )
...
def chunk_query(tm1: TM1Service, mdx: str | MdxBuilder, chunk_size: int = 1000) -> list[str]:
    ...
    for row_dim_set in mdx_builder.axes.get(1, MdxAxis.empty()).dim_sets:
        ...
        subset = AnonymousSubset(row_dim_set.dimension, row_dim_set.hierarchy, elements=elements)
    ...
    for col_dim_set in mdx_builder.axes.get(0, MdxAxis.empty()).dim_sets:
        ...
        subset = AnonymousSubset(col_dim_set.dimension, col_dim_set.hierarchy, elements=element_names)
    ...
    for title in mdx_builder._where.members:
        subset = AnonymousSubset(title.dimension, title.hierarchy, elements=[title.element])
    ...
    for dim_name, subset in row_set.items():
        ...
        for i in range(0, len(subset.elements), batch):
            chunk_row_set.append(
                AnonymousSubset(dim_name, subset.hierarchy_name, elements=subset.elements[i : i + batch])
            )
    ...
    for dim_name, subset in col_set.items():
        ...
        for i in range(0, len(subset.elements), chunk_size):
            chunk_col_set.append(
                AnonymousSubset(dim_name, subset.hierarchy_name, elements=subset.elements[i : i + chunk_size])
            )
